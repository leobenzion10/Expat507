...
-- Tool leads (interactive tools: visa quiz, cost calculator, investment simulator, visa comparator)
create table if not exists tool_leads (
  ...
  language   text not null default 'es' check (language in ('es', 'en')),
  result     jsonb,
  source     text not null default 'herramientas',
  created_at timestamptz not null default now()
);

create index if not exists tool_leads_email_idx on tool_leads (email);
create index if not exists tool_leads_tool_idx on tool_leads (tool);
...
-- Public can insert tool_leads
create policy "Public can insert tool_leads"
  on tool_leads for insert to anon with check (true);
